...
{
    private const float MAX_DEGREE = 359.999f;
    ...
    [Tooltip("해당 각도가 플레이어가 최대로 병을 기울일 수 있는 각도임")]
    ...
    [field: SerializeField, OverrideLabel("병의 기본 기울기 각도(degree)"), Foldout("각도"), Range(0f, MAX_DEGREE)]
    [Tooltip("계량을 시작했을 때, 기본적인 병의 회전 각도. 이 값이 0일 경우, 병은 수직으로 세워져 있게됨\n'병 기울임 각도(degree)'를 넘지 않도록 유의")]
    private float _defaultAngle = 40f;

    [field: SerializeField, OverrideLabel("지거로부터 병 입구의 거리(m)"), Foldout("기타"), MinValue(0f)]
    [Tooltip("병의 입구 피봇 위치와 지거의 피봇 위치와의 y축 offset")]
    private float _jiggerToDistance = 0f;
    ...
    public float JiggerToDistance => _jiggerToDistance;
